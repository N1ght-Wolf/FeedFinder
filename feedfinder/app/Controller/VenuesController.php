...
class VenuesController extends AppController
{
    ...
    public function get_review(){
      ...
      if ($this->request->is('ajax')) {
          ...
          $result = $this->Review->getReviewPaginated($query);

          if($query['rating_high'] == -1 && $query['rating_low'] == -1)
          {
            $performances = array(
              'terrible' =>$this->Review->getVenuePerformance($query,0,1),
              'poor'=>$this->Review->getVenuePerformance($query,1,2),
              'average'=>$this->Review->getVenuePerformance($query,2,3),
              'vgood'=>$this->Review->getVenuePerformance($query,3,4),
              'excellent'=>$this->Review->getVenuePerformance($query,4,6)
            );

          }else{
            $performances=array(
              $query['rating_name']=>$this->Review->getVenuePerformance($query,$query['rating_low'],$query['rating_high'])
            );
          }
          $json = array('paginated' => $result,'performances'=>$performances );
