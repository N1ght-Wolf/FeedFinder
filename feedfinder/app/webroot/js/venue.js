...
$(document).ready(function () {
    ...
    $(performanceLinks).each(function () {
        $(this).click(function (event) {
            ...
            var id = $(this).attr('id')
            query.rating_name=id.substring(0,id.indexOf('-'));
...
function showPerformance(perf) {
    ...
    venue.performances.veryGood = calcPercentage(perf.performances.vgood);
...
function getReview(query) {
    return $.ajax({
        url: getBaseURL() + 'feedfinder/venues/get_reviews',
        data: query,
        dataType: 'json'
    });
}
...
function getReviews(formData) {
    return $.ajax({
        url: getBaseURL() + 'feedfinder/venues/get_review',
        data: formData,
        dataType: 'json'
    });
}
...
function getReviewCount(query) {
    return $.ajax({
        url: getBaseURL() + 'feedfinder/venues/get_count',
        data: {
            query: query
        },
        dataType: 'json'
...
function getRatings(query) {
    return $.ajax({
        url: getBaseURL() + 'feedfinder/venues/get_venue_ratings',
        data: query,
        dataType: 'json'
    })
}
...
function getPerformance(formData) {
    $.ajax({
        url: getBaseURL() + 'feedfinder/venues/get_venue_perform',
        data: formData,
        dataType: 'json'
    }).done(function (data) {
        updateProgBar(data);
...
function calcPercentage(val) {
    if (numberOfReviews > 0) {
        var result = (val / numberOfReviews) * 100;
        if(isNaN(result)){
          result = 0;
        }
        return result;
