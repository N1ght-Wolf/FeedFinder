    ...
    function getDateRange(pos, format) {
        ...
        switch (pos) {
            ...
            case 8: //this year
                ...
                break;
            case 9: // lifetime
                currentDateSelection = {from: moment("2013-1-1").format(format),to: moment().format(format)};
                return {
    ...
    function placesSubmit(){
        ...
        $.when(
            ...
            $.ajax({
                ...
                console.log(markers);
                drawVenueMarkers(wms[0]['cluster_data']);
    ...
    function drawVenueMarkers(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            title =
                "<div class='span4'>" +
                '<h4>' + venue.name + '</h4>' +
                '<a target="_blank" href=http://localhost/feedfinder-local/venues?id=' + venue.id +
                '&from=' + dateRange.from + '&to=' + dateRange.to + '>' + review.length + ' review(s)</a>' +
                '<address>';
            if(venue.address != null){
                title= title+'<strong>' + venue.address + '</strong><br>';
            }
            if(venue.city!=null){
                title = title+venue.city + '<br>';
            }
            if(venue.postalCode!=null){
                title = title+ venue.postalCode + '<br>';
            }
            title=title+ venue.created + '<br>' +
                '</address>' +
                "</div>";
    ...
    function drawMarker(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            //if (count > 0) {
                marker = L.marker(new L.latLng(venue.latitude, venue.longitude), {
